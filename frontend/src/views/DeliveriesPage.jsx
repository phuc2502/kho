    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button
            onClick={() => {
              const headers = ['Mã phiếu', 'Khách hàng', 'Từ yêu cầu', 'Ngày tạo', 'Tổng tiền', 'Trạng thái'];
            ...
            className="flex items-center gap-1.5 px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-sm font-semibold animate-in fade-in duration-200"
          >
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        @media print {
          ...
          #delivery-print-canvas, #delivery-print-canvas * {
            visibility: visible;
          }
          #delivery-print-canvas {
      ...
      {/* Deliveries Printable Canvas */}
      {selectedDelivery && (
        <div id="delivery-print-canvas" className="hidden p-8 bg-white text-black font-serif">
          ...
          <div className="text-center my-6 font-serif">
            ...
            <p className="italic mt-1 text-xs">Ngày xuất: {selectedDelivery.createdAt ? new Date(selectedDelivery.createdAt).toLocaleDateString('vi-VN') : '—'}</p>
            <p className="text-xs font-mono mt-0.5">Mã phiếu: {selectedDelivery.code}</p>
          </div>

          <div className="space-y-1 mb-6 font-serif text-xs">
            <p><span className="font-bold">Khách hàng:</span> {selectedDelivery.tenKhachHang}</p>
            <p><span className="font-bold">Người lập phiếu:</span> {selectedDelivery.createdByUser?.username}</p>
            <p><span className="font-bold">Từ yêu cầu:</span> {selectedDelivery.fromRequest?.code || 'Trực tiếp'}</p>
            <p><span className="font-bold">Trạng thái phiếu:</span> {selectedDelivery.status}</p>
          </div>
          ...
          <table className="w-full text-left font-serif text-xs border border-collapse border-black text-black">
            ...
            <tbody>
              ...
              <tr className="font-bold bg-slate-50">
                <td colSpan="6" className="border border-black text-right px-2 py-1.5 uppercase">Tổng cộng:</td>
                <td className="border border-black text-right px-2 py-1.5">{selectedDelivery.totalAmount?.toLocaleString('vi-VN')} đ</td>
              </tr>
            </tbody>
          </table>
          ...
          <div className="grid grid-cols-2 text-center mt-12 text-xs font-serif">
            ...
            <div>
              <p className="font-bold">Thủ kho / Người nhận</p>
              <p className="italic text-[10px] text-slate-500">(Ký, họ tên)</p>
            </div>
          </div>
        </div>
