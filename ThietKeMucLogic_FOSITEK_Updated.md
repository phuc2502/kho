...
> ---
> ## ⭐ NHẬT KÝ CẬP NHẬT v2.0 (Cải tiến theo phân tích Gap Sacomtec)
>
> | # | Loại | Bảng / Phần | Nội dung thay đổi |
> |---|------|------------|-------------------|
> | 1 | 🆕 THÊM MỚI | **LoSanXuat** | Bảng Lô sản xuất (Lot Number) – truy xuất nguồn gốc & thu hồi lô |
> | 2 | 🔧 CẢI TIẾN | **SanPham** | + trường `Thoi_han_bao_hanh` (hạn bảo hành theo tháng) |
> | 3 | 🔧 CẢI TIẾN | **SoSerial** | + `Ma_lo` FK → LoSanXuat, + `Han_bao_hanh` DATE |
> | 4 | 🔧 CẢI TIẾN | **ChiTietNhapKho** | + `Ma_lo` FK → LoSanXuat (ghi lô tại thời điểm nhập) |
> | 5 | 🔧 CẢI TIẾN | **NhatKyEmail** | Mở rộng `Loai_email` + thêm `Ma_san_pham`, `Da_xu_ly` cho cảnh báo nghiệp vụ |
> | 6 | 📐 GHI CHÚ | **Xuất kho – FIFO** | Quy tắc nghiệp vụ + Index hỗ trợ FIFO theo lô |
> | 7 | 📊 GHI CHÚ | **Trang tổng quan** | Mô tả 5 widget báo cáo trên Dashboard |
>
> **Tổng bảng sau cập nhật: 34** (32 gốc + NhatKyEmail + LoSanXuat)
> ---
...
| **17⭐** | **LoSanXuat** 🆕 | **[THÊM MỚI v2.0]** Lô sản xuất (Lot Number): nhóm serial theo đợt SX, hỗ trợ FIFO & thu hồi lô lỗi |
...
| **Thoi\_han\_bao\_hanh** 🆕 | *INT* | NULL, DEFAULT NULL | ***[THÊM MỚI v2.0]** Thời hạn bảo hành tính bằng tháng. NULL = không bảo hành. VD: 12 / 24 / 36* |
...
\* Chỉ tồn tại khi SanPham.Quan\_ly\_theo\_serial \= 1\. Trạng thái: Trong kho / Đã xuất / Lỗi / Hủy. **[⭐ v2.0]** Bổ sung Ma\_lo (FK → LoSanXuat) và Han\_bao\_hanh để hỗ trợ FIFO theo lô và cảnh báo sắp hết bảo hành.
...
| **Ma\_lo (FK)** 🆕 | *CHAR(15)* | NULL, FK → LoSanXuat | ***[THÊM MỚI v2.0]** Lô sản xuất chứa serial này. NULL nếu sản phẩm không theo dõi lô* |
| **Han\_bao\_hanh** 🆕 | *DATE* | NULL | ***[THÊM MỚI v2.0]** Ngày hết hạn bảo hành của đơn vị này. Tính bằng: Ngay\_san\_xuat + SanPham.Thoi\_han\_bao\_hanh tháng* |
...
\* So\_luong\_thuc\_te do NV kho đếm, So\_luong\_nhap từ chứng từ – chênh lệch sẽ phát sinh PhieuSuCo. Không lưu Thoi\_gian\_tao/cap\_nhat vì bảng này chỉ ghi nhận tại thời điểm nhập phiếu. **[⭐ v2.0]** Bổ sung Ma\_lo để liên kết dòng nhập với lô sản xuất cụ thể, hỗ trợ truy xuất nguồn gốc toàn bộ lô khi có sự cố.
...
| **Ma\_lo (FK)** 🆕 | *CHAR(15)* | NULL, FK → LoSanXuat | ***[THÊM MỚI v2.0]** Lô sản xuất tương ứng với dòng nhập này. Dùng để trace: "Lô LSX-001 được nhập qua phiếu nào?"* |
...
> **[⭐ CẢI TIẾN v2.0]** Mở rộng từ bảng log xác thực thuần túy thành **unified notification log**: ghi nhận cả email xác thực tài khoản lẫn **cảnh báo nghiệp vụ** (tồn kho, bảo hành, hàng tồn lâu). Bổ sung 3 trường: `Ma_san_pham`, `Ma_vi_tri`, `Da_xu_ly`.
...
| Loai\_email *(type)* 🔧 | ENUM | NOT NULL | welcome / adminReset / forgotPassword / **stockLow** / **stockHigh** / **warrantyExpiring** / **slowMoving** *(⭐ 4 giá trị mới v2.0)* |
...
| **Ma\_san\_pham** 🆕 | CHAR(15) | NULL, FK → SanPham | ***[THÊM MỚI v2.0]** Sản phẩm liên quan — chỉ điền cho loại cảnh báo nghiệp vụ* |
| **Ma\_vi\_tri** 🆕 | CHAR(15) | NULL, FK → ViTriTrongKho | ***[THÊM MỚI v2.0]** Vị trí kho liên quan đến cảnh báo (nếu có)* |
| **Da\_xu\_ly** 🆕 | BIT | DEFAULT 0 | ***[THÊM MỚI v2.0]** 0: Chưa xử lý / 1: Đã xem và xử lý — dùng cho cảnh báo nghiệp vụ* |
...
- **stockLow** ⭐: Tồn kho dưới ngưỡng tối thiểu (`TonKho.So_luong_ton < So_luong_toi_thieu`) → `Ma_san_pham` + `Ma_vi_tri` được điền
- **stockHigh** ⭐: Tồn kho vượt ngưỡng tối đa (`So_luong_ton > So_luong_toi_da`) → `Ma_san_pham` + `Ma_vi_tri` được điền
- **warrantyExpiring** ⭐: Serial sắp hết bảo hành trong 30 ngày (`SoSerial.Han_bao_hanh <= GETDATE()+30`) → `Ma_san_pham` được điền
- **slowMoving** ⭐: Sản phẩm tồn kho quá 90 ngày chưa xuất → `Ma_san_pham` được điền
...
## 🆕 [THÊM MỚI v2.0] Bảng LoSanXuat (ProductionLot) – Nhóm 3 bổ sung

> **Lý do thêm mới:** Linh kiện điện tử FOSITEK được sản xuất theo từng đợt (lot/batch). Mỗi đợt mang số lô riêng của nhà máy. Nếu phát hiện lỗi kỹ thuật ở 1 lô, hệ thống cần có khả năng **truy xuất toàn bộ serial thuộc lô đó đang ở đâu trong kho** để xử lý thu hồi. Ngoài ra, lô giúp **thực thi FIFO**: xuất lô nhập trước, tránh sản phẩm tồn kho lâu ngày.

**Bảng: LoSanXuat**

\* Một lô sản xuất (Lot) là một đợt sản xuất cụ thể, thường gắn với cùng nguyên vật liệu, cùng line sản xuất, cùng ca. Một sản phẩm có thể có nhiều lô; một lô chỉ thuộc một sản phẩm. Quan hệ: **SanPham (1) → LoSanXuat (N) → SoSerial (N)**.

| Tên thuộc tính | Kiểu dữ liệu | Ràng buộc | Mô tả / Ví dụ |
| :---- | :---- | :---- | :---- |
| **Ma\_lo (PK)** | *CHAR(15)* | NOT NULL, UNIQUE | *LSX-2026-001* |
| Ma\_san\_pham (FK) | *CHAR(15)* | FK → SanPham, NOT NULL | *Lô này thuộc sản phẩm nào* |
| So\_lo\_nha\_may | *NVARCHAR(50)* | NOT NULL | *Số lô của nhà máy, VD: LOT-A0326-02* |
| Ngay\_san\_xuat | *DATE* | NOT NULL | *2026-03-15 – ngày sản xuất lô* |
| So\_luong\_san\_xuat | *INT* | DEFAULT 0 | *1000 – tổng đơn vị trong lô* |
| Nguon\_goc | *NVARCHAR(200)* | NULL | *Line 2 – Xưởng A, ca sáng* |
| Ghi\_chu | *NVARCHAR(300)* | NULL | *Ghi chú QC đặc biệt nếu có* |
| Thoi\_gian\_tao | *DATETIME* | DEFAULT GETDATE() | *Tự động khi tạo lô* |

**Quan hệ LoSanXuat với các bảng khác:**

| Bảng | Loại quan hệ | Ý nghĩa |
| :---- | :---- | :---- |
| SanPham | N:1 | Nhiều lô thuộc 1 sản phẩm |
| SoSerial | 1:N | 1 lô chứa nhiều serial (từng đơn vị) |
| ChiTietNhapKho | 1:N | Ghi nhận lô khi nhập vào kho |

**Index gợi ý:**

| Index | Cột | Mục đích |
| :---- | :---- | :---- |
| IDX\_LoSanXuat\_SanPham | (Ma\_san\_pham, Ngay\_san\_xuat) | Tra cứu lô theo sản phẩm, sort FIFO |
| IDX\_SoSerial\_Lo | (Ma\_lo, Trang\_thai) | Đếm serial còn trong kho theo lô |

---

## ⭐ [GHI CHÚ v2.0] Quy tắc FIFO trong xuất kho

> **Vấn đề đặt ra:** Hệ thống có serial tracking và lot tracking, nhưng cần đảm bảo nhân viên kho xuất đúng thứ tự lô cũ → lô mới (First In First Out).

**Quy tắc nghiệp vụ (Business Rule):**

Khi tạo `PhieuXuatKho`, bộ phận kho phải ưu tiên chọn serial theo thứ tự:

```sql
-- Gợi ý serial FIFO cho sản phẩm cần xuất
SELECT s.Ma_serial, s.Ma_lo, l.Ngay_san_xuat, s.Han_bao_hanh
FROM SoSerial s
JOIN LoSanXuat l ON s.Ma_lo = l.Ma_lo
WHERE s.Ma_san_pham = @ma_san_pham
  AND s.Trang_thai  = N'Trong kho'
ORDER BY l.Ngay_san_xuat ASC,   -- Lô cũ nhất trước (FIFO)
         s.Han_bao_hanh   ASC,   -- Nếu cùng lô, serial hết hạn sớm trước (FEFO)
         s.Ma_serial      ASC
```

**Index bổ sung trên SoSerial:**

```sql
CREATE INDEX IDX_SoSerial_FIFO
ON SoSerial (Ma_san_pham, Ma_lo, Trang_thai)
INCLUDE (Han_bao_hanh, Ngay_san_xuat);
```

**Ghi chú triển khai:**
- Hệ thống hiển thị danh sách serial gợi ý FIFO; nhân viên kho có thể chọn khác nếu có lý do đặc biệt (ghi lại vào Ghi\_chu của ChiTietXuatSerial).
- Cột `Han_bao_hanh` trong `SoSerial` được tính tự động bằng trigger hoặc application khi tạo serial: `Han_bao_hanh = DATEADD(MONTH, SanPham.Thoi_han_bao_hanh, SoSerial.Ngay_san_xuat)`.

---

## 📊 [GHI CHÚ v2.0] Trang Tổng Quan (Dashboard) – Báo cáo phân tích

> **Nguồn dữ liệu:** Tất cả widget đều tính từ dữ liệu hiện có trong các bảng, không cần bảng riêng.

| Widget | Nguồn dữ liệu | Điều kiện lọc |
| :---- | :---- | :---- |
| 📦 **Tốc độ tiêu thụ** (30/60/90 ngày) | `ChiTietXuatKho` GROUP BY `Ma_san_pham`, tháng | `PhieuXuatKho.Ngay_xuat >= GETDATE()-N` |
| ⚠️ **Sản phẩm dưới ngưỡng tối thiểu** | `TonKho` JOIN `SanPham` | `So_luong_ton < So_luong_toi_thieu` |
| 🔔 **Serial sắp hết bảo hành** (<30 ngày) | `SoSerial` JOIN `SanPham` | `Trang_thai='Trong kho' AND Han_bao_hanh BETWEEN GETDATE() AND GETDATE()+30` |
| 🕰️ **Hàng tồn lâu ngày** (>90 ngày) | `SoSerial` JOIN `LoSanXuat` | `Trang_thai='Trong kho' AND LoSanXuat.Ngay_san_xuat < GETDATE()-90` |
| 📊 **Top 10 sản phẩm xuất nhiều nhất** | `ChiTietXuatKho` GROUP BY `Ma_san_pham` | Tháng hiện tại |

**Cảnh báo tự động (qua NhatKyEmail):**
- Hệ thống chạy job định kỳ (hằng ngày) để kiểm tra 4 điều kiện trên.
- Nếu phát hiện vi phạm → INSERT vào `NhatKyEmail` với `Da_xu_ly = 0`.
- Quản lý kho nhận email + thấy badge số lượng chưa xử lý trên Dashboard.
- Sau khi xử lý → cập nhật `Da_xu_ly = 1`.
